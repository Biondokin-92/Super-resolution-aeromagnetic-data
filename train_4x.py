...
parser.add_argument('--data_dir', type=str, default='/content/drive/MyDrive/rcan-tensorflow/TTST-main/AID-tiny/Train')
parser.add_argument('--val_dir', type=str, default='/content/drive/MyDrive/rcan-tensorflow/TTST-main/AID-tiny/valid')
...
parser.add_argument('--residual', type=bool, default=False, help='Use global residual or not')
parser.add_argument('--pretrained_sr', default='saved_models/ttst/xx.pth', help='sr pretrained base model')
parser.add_argument('--pretrained', type=bool, default=False)
parser.add_argument('--save_folder', default='s/content/drive/MyDrive/rcan-tensorflow/model_edsr', help='Location to save checkpoint models')
parser.add_argument('--log_folder', default='/content/drive/MyDrive/rcan-tensorflow/model_edsr/', help='Location to save checkpoint models')
...
L1_criterion = nn.L1Loss().cuda(gpus_list[0])
...
for epoch in range(opt.start_epoch, opt.nEpochs + 1):
    ...
    for iteration, batch in enumerate(training_data_loader, 1):
        ...
        epoch_loss += loss.item()  # Removed unwanted 'a' at the end
        loss.backward()
        optimizer.step()
    ...
    for batch in val_data_loader:
        ...
        prediction = prediction.cpu().data[0].numpy().astype(np.float32) * 255.0
        prediction = prediction.clip(0, 255)

        gt = gt.cpu().squeeze().numpy().astype(np.float32) * 255.
    ...
    print_log("===> Epoch {} Complete: Avg. PSNR: {:.4f} Best Epoch {} Best PSNR: {:.4f}".format(epoch, avg_psnr_predicted / len(val_data_loader), best_epoch, best_test_psnr))
